...
export async function GET(request, {params}){
  ...
  try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/${id}/dishes`,{
          headers: {
...
export async function POST(req, { params }) {
  try {
    ...
    const { categorieId, nom, description, prix } = await req.json(); // Lire les données envoyées
    const token = req.headers.get("Authorization")?.split(" ")[1];
    // Validation des champs obligatoires
    if (!categorieId || !nom || !prix || !description) {
      return NextResponse.json({ 
        success: false, 
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_BASE_URL}/${id}/dishes`,
      {
        method: "POST",
        headers: {
          'Authorization': `Bearer ${token}`, 
    ...
    console.log('VVVVV',dataToReturn)
    return NextResponse.json({ 
      success: true, 
